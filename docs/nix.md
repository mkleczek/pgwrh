...
`pg_background`, `pgwrh`, `pgwrh_fdw`, and `pgwrh_wait` available.
The three pgwrh extensions share version 0.3.0. Nix must have flakes enabled.
...
wait API remains usable, all three extension versions, and both native libraries.
...
```nix
inputs.pgwrh.url = "github:mkleczek/pgwrh/v0.3.0";
```
