...
A user must have USAGE on the selected member and a user-specific or PUBLIC
mapping there. A PUBLIC mapping on the virtual server does not grant access to
its members. Role-specific mappings take precedence; role membership does not
make another role's mapping applicable. Access through a view uses the effective
user for that view. If no member is accessible, the query fails.
