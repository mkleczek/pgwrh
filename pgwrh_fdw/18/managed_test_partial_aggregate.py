...
def test_partial_aggregation_in_managed_topology(postgres_node_factory):
    ...
    for reader in cluster.replicas:
        ...
        with reader.node.connect() as conn:
            ...
            for query in [
                'SELECT count(*),count(v),sum(v),min(v),max(v),avg(v),avg(v::smallint) FROM data.root',
                'SELECT g,count(*),sum(v) FILTER(WHERE v>40),min(v),max(v),avg(v) FILTER(WHERE v>40),avg(v::smallint) '
                ...
                assert foreign[0]['Remote SQL'].count('ARRAY[') == 2, tree
                assert 'avg(' not in foreign[0]['Remote SQL'], tree
