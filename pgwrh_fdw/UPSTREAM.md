...
The source files in `pgwrh_fdw/` fork `contrib/postgres_fdw`. Retained upstream
tests and local test helpers live in `test/pgwrh_fdw/`.
...
`pgwrh_fdw/COPYRIGHT` is PostgreSQL's license, copied from the same release.
...
```sh
git subtree merge --prefix=pgwrh_fdw upstream/postgres_fdw
```

Use subtree merges without `--squash`; an ordinary merge would target the wrong
directory. Run with a clean working tree. Reconcile upstream test and Makefile
changes with the tests now located in `test/pgwrh_fdw/`; keep build definitions
in `pgwrh_fdw/Makefile` and test definitions in `test/pgwrh_fdw/Makefile`.
Resolve update conflicts, review upstream API changes, update this provenance
file and `pgwrh_fdw/COPYRIGHT`, and run
