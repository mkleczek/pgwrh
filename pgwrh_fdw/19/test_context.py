...
class ContextTests(unittest.TestCase):
    ...
    def test_initial_release_installation(self):
        self.assertEqual(self.c.scalar(
            "SELECT extversion FROM pg_extension WHERE extname = 'pgwrh_fdw'"), "1.0.0-alpha1")
        ...
        self.assertEqual(self.c.scalar(
            "SELECT version FROM pg_get_loaded_modules() WHERE module_name = 'pgwrh_fdw'"), "1.0.0-alpha1")
    ...
    def test_remote_invalid_value_poisons_failed_initialization(self):
        ...
        self.error("SELECT * FROM a", "08000", "abort cleanup failure")
        self.c.sql("ROLLBACK")
