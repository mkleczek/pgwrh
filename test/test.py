    ...
    def test_temp(self):
        ...
        with scoped_config(use_python_logging=True):
            with get_new_node().init(allow_logical=True) as master:
                master.append_conf('max_worker_processes = 100').append_conf('max_replication_slots = 100').append_conf('max_wal_senders = 100')
                master.start()
                ...
                with get_new_node().init() as replica1, get_new_node().init() as replica2, get_new_node().init() as replica3, get_new_node().init() as replica4:
                    ...
                    for replica in replicas:
                        print(f'Starting replica {replica[1]}')
                        ...
                        print(f'Starting replica worker {replica[1]}')
