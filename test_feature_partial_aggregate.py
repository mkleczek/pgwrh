...
class PartialAggregateTests(unittest.TestCase):
    ...
    def test_grouped_and_ungrouped(self):
        for group in ['', ' GROUP BY g ORDER BY g NULLS FIRST']:
            ...
            self.assert_query('SELECT ' + prefix +
                'count(*),count(v),sum(v),min(v),max(v),avg(v),avg(v::smallint),min(label),max(label) FROM items' + group)
    ...
    def test_mixed_local_foreign(self):
        ...
        for group in ['', ' GROUP BY g ORDER BY g NULLS FIRST']:
            plan = self.assert_query('SELECT ' + ('g,' if group else '') +
                'count(*),count(v),sum(v),min(v),max(v),avg(v),avg(v::smallint) FROM items' + group, scans=2)
            self.assertTrue(any(n.get('Partial Mode') == 'Partial' for n in self.nodes(plan)))

    def test_integer_avg_state_and_unequal_shard_counts(self):
        self.c.sql('TRUNCATE baseline,stored0,stored1,stored2; '
                   'INSERT INTO baseline(k,g,v) VALUES '
                   '(1,1,10),(2,1,20),(3001,1,60),(3,2,NULL),(3002,2,NULL); '
                   'INSERT INTO stored0 SELECT * FROM baseline WHERE k<3000; '
                   'INSERT INTO stored1 SELECT * FROM baseline WHERE k>=3000')
        for group in ['', ' GROUP BY g ORDER BY g']:
            query = 'SELECT ' + ('g,' if group else '') + 'avg(v),avg(v::smallint),sum(v),count(*) FROM items' + group
            plan = self.assert_query(query)
            for node in self.nodes(plan):
                if 'Remote SQL' in node:
                    sql = node['Remote SQL']
                    self.assertIn('ARRAY[count(v), COALESCE(sum(v), 0::bigint)]', sql)
                    self.assertEqual(sql.count('ARRAY['), 2, sql)
                    self.assertNotIn('avg(', sql)
        # Averaging the two shard averages would incorrectly return 37.5.
        self.assertEqual(Decimal(self.c.scalar('SELECT avg(v) FROM items')), Decimal(30))
        self.assertEqual(self.c.scalar('SELECT avg(v) FROM items WHERE g=2'), None)
        query = 'SELECT avg(v) FILTER(WHERE k<>2),avg(v::smallint) FILTER(WHERE k<>2) FROM items'
        plan = self.assert_query(query)
        for node in self.nodes(plan):
            if 'Remote SQL' in node:
                self.assertEqual(node['Remote SQL'].count('FILTER (WHERE'), 4, node)
        self.assertEqual(Decimal(self.c.sql(query)[0][0]), Decimal(35))

    def test_integer_avg_bounds_and_shared_aggregates(self):
        self.c.sql('TRUNCATE baseline,stored0,stored1,stored2; '
                   'INSERT INTO baseline(k,g,v) VALUES '
                   '(1,-32768,-2147483648),(2,32767,2147483647),'
                   '(3001,32767,2147483647),(3002,-32768,-2147483648); '
                   'INSERT INTO stored0 SELECT * FROM baseline WHERE k<3000; '
                   'INSERT INTO stored1 SELECT * FROM baseline WHERE k>=3000')
        query = 'SELECT avg(v),sum(v),count(v),avg(v),avg(g::smallint),sum(g::smallint),count(g) FROM items'
        self.assert_query(query)
        result = self.c.sql(query)[0]
        self.assertEqual(Decimal(result[0]), Decimal('-0.5'))
        self.assertEqual(Decimal(result[4]), Decimal('-0.5'))
    ...
    def test_supported_signatures(self):
        ...
        for i, (typename, expr) in enumerate(values.items()):
            with self.subTest(type=typename):
                ...
                if typename in ('smallint', 'integer'):
                    aggregates += f',avg(x{i})'
    ...
    def test_filter_having_and_projection(self):
        for query in [
            ...
            'SELECT g FROM items GROUP BY g HAVING avg(v)>0 ORDER BY g',
            'SELECT g,count(*) FROM items GROUP BY g HAVING local_keep(avg(v)::int) ORDER BY g',
            'SELECT avg(v) FROM items HAVING avg(v)>0',
            'SELECT avg(v) FILTER(WHERE false),avg(v::smallint) FILTER(WHERE NULL) FROM items',
            ...
            'SELECT g+1,count(*),sum(v),avg(v+1),avg(v::smallint)+avg(v) FROM items WHERE v>0 GROUP BY g+1 ORDER BY 1',
    ...
    def test_empty_input_and_all_null_states(self):
        for suffix in [' WHERE v IS NULL', ' WHERE v>10000', ' WHERE k>=6000']:
            ...
            self.assert_query('SELECT count(*),count(v),sum(v),min(v),max(v),avg(v),avg(v::smallint) FROM items'+suffix, scans=scans)
            self.assert_query('SELECT g,count(v),sum(v) FROM items'+suffix+
                              ' GROUP BY g ORDER BY g NULLS FIRST', scans=scans)
        self.c.sql('TRUNCATE baseline,stored0,stored1,stored2')
        self.assert_query('SELECT count(*),count(v),sum(v),min(v),max(v),avg(v),avg(v::smallint) FROM items')
        self.assert_query('SELECT g,count(*) FROM items GROUP BY g ORDER BY g')

    def test_unsupported_and_mixed_fallback(self):
        for expression in ['avg(v::bigint)', 'avg(v::numeric)', 'avg(v::real)',
                           "avg(v*interval '1 second')", 'avg(DISTINCT v)',
                           'avg(v ORDER BY k)', 'sum(v::bigint)', 'sum(v::numeric)',
                           "sum(v*interval '1 second')", 'avg(v::float8)',
                           'array_agg(v ORDER BY k)', 'count(DISTINCT v)', 'sum(v ORDER BY k)',
                           'min(ARRAY[v])']:
            with self.subTest(expression=expression):
                self.assert_query('SELECT g,count(*),avg(v),'+expression+
                                  ' FROM items GROUP BY g ORDER BY g', pushed=False)
        self.assert_query('SELECT g,count(*) FROM items GROUP BY g HAVING avg(v::numeric)>0 ORDER BY g', pushed=False)
        self.assert_query('SELECT g,avg(v) FILTER(WHERE local_keep(v)) FROM items GROUP BY g ORDER BY g', pushed=False)
        self.assert_query('SELECT g,avg(local_keep(v)::int) FROM items GROUP BY g ORDER BY g', pushed=False)
    ...
    def test_same_name_custom_aggregate(self):
        ...
        self.c.sql("""CREATE AGGREGATE public.avg(integer) (sfunc=int4_avg_accum,
            stype=bigint[], combinefunc=int4_avg_combine, finalfunc=int8_avg,
            initcond='{0,0}', parallel=safe);
            ALTER EXTENSION pgwrh_fdw ADD AGGREGATE public.avg(integer)""")
        self.assert_query('SELECT g,count(*),public.avg(v) FROM items GROUP BY g ORDER BY g', pushed=False)
    ...
    def test_prepared_custom_and_generic(self):
        query = 'SELECT g,count(*),sum(v) FILTER(WHERE v>$1),avg(v) FILTER(WHERE v>$1),avg($1) FROM items WHERE k>=$2 GROUP BY g HAVING count(*)>$3 ORDER BY g'
    ...
    def test_control_remote_estimates_and_full_aggregation(self):
        query='SELECT g,count(*),sum(v),avg(v),avg(v::smallint) FROM items GROUP BY g ORDER BY g'
        ...
        query='SELECT k,count(*),avg(v),avg(v::numeric) FROM items GROUP BY k HAVING count(*)>0 ORDER BY k'
        plan=self.plan(query)
        sqls=[n['Remote SQL'] for n in self.nodes(plan) if 'Remote SQL' in n]
        self.assertTrue(all('avg(' in s and ' HAVING ' in s for s in sqls), sqls)
        self.assertTrue(all('ARRAY[' not in sql for sql in sqls), sqls)
