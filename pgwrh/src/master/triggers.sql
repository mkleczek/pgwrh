...
BEGIN
    IF EXISTS (SELECT 1 FROM
        "@extschema@".missing_connected_local_shard
               WHERE
                   replication_group_id = NEW.replication_group_id AND version = NEW.current_version
    ) THEN
    ...
    END IF;
    IF EXISTS (SELECT 1 FROM
        "@extschema@".missing_ready_remote_shard
               WHERE
                   replication_group_id = NEW.replication_group_id AND version = NEW.current_version
    ) THEN
        RAISE 'Not all hosts confirmed configuration of required remote shards'
            USING HINT = 'Check missing_ready_remote_shard view for details';
    END IF;
    RETURN NEW;
END;
$$;
...
'Commit requires target local copies and remote routes to be ready in this group.
A retained connected local copy may stand in for its prepared remote replacement;
readers using remote routes must have switched to the target destination.';
...
$$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM "@extschema@".replication_group_config_lock
                   WHERE replication_group_id = NEW.replication_group_id AND rollback_unlock IS NOT NULL) THEN
        RETURN NULL;
    END IF;
    IF EXISTS (
        SELECT 1 FROM "@extschema@".replication_group g
        WHERE g.replication_group_id = NEW.replication_group_id
            AND g.current_version = g.target_version
            AND NOT EXISTS (SELECT 1 FROM "@extschema@".missing_connected_local_shard s
                WHERE (s.replication_group_id, s.version) = (g.replication_group_id, g.current_version))
            AND NOT EXISTS (SELECT 1 FROM "@extschema@".missing_ready_remote_shard s
                WHERE (s.replication_group_id, s.version) = (g.replication_group_id, g.current_version))
    ) THEN
        DELETE FROM "@extschema@".replication_group_config_lock
...
END
$$;
CREATE TRIGGER finish_rollback AFTER UPDATE ON replication_group_member
    FOR EACH ROW
    EXECUTE FUNCTION finish_rollback_trigger();
