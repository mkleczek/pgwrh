...
catalog's transition/output types, combine function, and absence of finalization
or state serialization. A same-named user aggregate (including one installed in
pg_catalog or declared shippable by an extension) is not admitted.

## Restrictions and planning

- `avg`, `sum(bigint)`, `sum(numeric)`, `sum(interval)` and all opaque `internal`
...
groups. The script asserts these counts, not an elapsed-time improvement. This
...
consumes the ordinary scalar results admitted here.
